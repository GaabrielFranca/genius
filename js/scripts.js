...
_gui.strict.addEventListener("click", () => {
	...
	let strictOn = _gui.led.classList.toggle("gui__led--active")
...
const padListener = (e) => {
	...
	_data.sounds[soundsId].play();
	_data.playerSequence.push(soundsId)
...
const setScore = () => {
	...
	_gui.counter.innerHTML = "0" + score

	if (score >= 10) {
		_gui.counter.innerHTML = score
	}
}
...
const newColor = () => {
	...
	_data.gameSequence.push(Math.round(Math.random() * 3))
	_data.score++
...
const blink = (text, callback) => {
	...
	const interval = setInterval(() => {
		if (!_data.gameOn) {
			clearInterval(interval)
			_gui.counter.classList.remove("gui__conunter--on")
			return
		}
